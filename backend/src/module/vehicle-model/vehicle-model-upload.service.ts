...
@Injectable()
export class VehicleModelUploadService {
  private readonly uploadPath = path.join(process.cwd(), 'public/images/vehicle-models');
  ...
  getFolderName(modelName: string): string {
    return modelName
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '');
  ...
  getImagePath(folderName: string, filename: string): string {
    return `images/vehicle-models/${folderName}/${filename}`;
  }
  ...
  saveBase64Images(modelName: string, base64Images: string[]): string[] {
    if (!base64Images || base64Images.length === 0) {
      return [];
    }

    const folderName = this.getFolderName(modelName);
